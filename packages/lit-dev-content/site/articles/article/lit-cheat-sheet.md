...
The current implementation as of December 2024 does not have a way to clean up
an effect. This means that if you create an effect, it will run indefinitely,
even if the Lit element is removed from the DOM. Additionally, any objects
referenced inside the effect will not be garbage collected since the effect is
always listening and running.

Until the `signal-utils` package provides a way to clean up effects, `effect()`
should be used with caution and likely outside of a component instance.
